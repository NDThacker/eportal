...
{
    public partial class sem_result : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (co > 0)
                {
                    queryString = "SELECT Subject.code,Subject.name,res_att.obt_mark,res_att.total_mark,res_att.result FROM res_att join Subject on res_att.std_id=@std_id and Subject.code=res_att.sub_id and res_att.sem=@sem and Subject.dept_name=@dept";
                    ...
                    foreach(DataRow row in ds.Tables["rdata"].Rows)
                    {
                        ...
                        cell3.Text = row["obt_mark"].ToString();
                        trow.Cells.Add(cell3);

                        TableCell cell4 = new TableCell();
                        cell4.Text = row["total_mark"].ToString();
                        trow.Cells.Add(cell4);

                        TableCell cell5 = new TableCell();
                        cell5.Text = row["result"].ToString();
